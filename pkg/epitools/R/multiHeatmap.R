makeBreaks<-function(u,n=100) {
  ...
  #  v<-max(abs(c(q[1],q[2])))
    #list(breaks=seq(-v,v,length=n),colors=colorpanel(low="grey90",mid="grey50",high="grey10",n=n-1))
    #list(breaks=seq(-v,v,length=n),colors=colorpanel(low="green",mid="black",high="red",n=n-1))
  #  list(breaks=seq(q[1]-.001,q[2]+.001,length=n),colors=colorpanel(low="lightgrey",mid="green",high="darkblue",n=n-1))
  #} else 
  if (ncol(u)==6) {
    q<-quantile(as.numeric(u),p=c(.025,.975),na.rm=TRUE)
    v<-max(abs(c(q[1],q[2])))
    #list(breaks=seq(-v,v,length=n),colors=colorpanel(low="lightpink",mid="violet",high="darkred",n=n-1))
    list(breaks=seq(q[1]-.001,q[2]+.001,length=n),colors=colorpanel(low="lightgrey",mid="green",high="darkblue",n=n-1))
  } else {
    q<-quantile(as.numeric(u),p=c(.5,.95),na.rm=TRUE)
    v<-max(abs(c(q[1],q[2])))
    #list(breaks=seq(-v,v,length=n),colors=colorpanel(low="lightpink",mid="violetred",high="darkred",n=n-1))
    list(breaks=seq(-v,v,length=n),colors=colorpanel(low="lightgrey",mid="green",high="darkblue",n=n-1))
  }
}
...
multiHeatmap<-function(dataList,colourList,titles=NULL,main="",showColour=TRUE,xspace=1,cwidth=.5,ystarts=c(.05,.90,.925,.95,.98),rlabelcex=1,clabelcex=1,titlecex=1.2,maincex=1.5,scalecex=.7) {
  require(grid)
  if(length(xspace)==1)
