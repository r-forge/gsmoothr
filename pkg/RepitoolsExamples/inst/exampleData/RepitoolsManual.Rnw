...
It can then be loaded by calling the function \texttt{setupExamples()}, after the package has been loaded into R (see below).
...
Analyses of next-generation sequencing data, especially for epigenomic data, are in thier infancy.  Further implementations will be made available in the near future.  Experiments are encouraged to provide feedback on the implemented routines as well as suggestions for further procedures.
