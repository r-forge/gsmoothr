...
\section{Getting help for the \texttt{Repitools} package}

First, users should consult the documentation available within R.  For example, to access the documentation for the \texttt{cpgDensityCalc}, type \texttt{?cpgDensityCalc} after loading the package.  Alternatively, you may call \texttt{help.start()} and browse the help documents (for all installed packages) through a web browser.

For questions that are not discussed in the function documentation, \texttt{Repitools} has a searchable mailing list, which can be accessed from: \\
\href{https://lists.r-forge.r-project.org/pipermail/repitools-help/}{https://lists.r-forge.r-project.org/pipermail/repitools-help/}

Users can sign up for the mailing list through: \\
\href{https://lists.r-forge.r-project.org/cgi-bin/mailman/listinfo/repitools-help}{https://lists.r-forge.r-project.org/cgi-bin/mailman/listinfo/repitools-help}
...
takeSample <- function(rs, f=.1) {
  gdapply(rs, function(x) {
    p <- x[["+"]]
    n <- x[["-"]]
    list( "+"=sort(sample(p, f*length(p))), "-"=sort(sample(n, f*length(n))) )
  })
}
...
\section{Environment}
This user manual was built using \texttt{Sweave} with the following $\mathbb{R}$ environment:
