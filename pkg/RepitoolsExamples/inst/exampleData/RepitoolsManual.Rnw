...
The function \texttt{cpgDensityPlot} uses \texttt{cpgDensityCalc} on sequencing data to examine the CpG density of reads, which is a useful quality control step for methylated DNA enrichment experiments. 

<<cpgSeq, fig=TRUE>>=
cpgDensityPlot(rs, seqLen=300, organism=Hsapiens)
@
...
lookupT <- binPlots(csTS, coordinatesTable=genes, ordering=emE, plotType="line", nbins=10)
