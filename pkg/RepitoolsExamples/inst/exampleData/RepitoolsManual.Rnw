...
\texttt{Repitools} is an R package to assist with the analysis of datasets typically found in epigenetics research; that is, those of a high dimensionality. The main focus of the package is the analysis of tiling and expression arrays. Functionality for Affymetrix and Nimblegen platforms is present. To keep this manual concise, the example used will be on some Affymetrix data. However, the functions are simple to use on Nimblegen data too, usually with the only difference being an extra parameter being given. Consult the help page of each function for how to use it on Nimblegen data. Additionally, simple analysis of next - generation sequencing data is also possible. Also, there are many options for each function, and just the main options are covered in this user manual.
\\ \\
The abilities of the package will be illustrated through a series of examples.
...
Repitools uses the \texttt{aroma.affymetrix} package, which requires data and their annotations to be in a specific directory structure, which must be writable by the end-user. The \texttt{RepitoolsExamples} package has the included example data files laid out correctly, and can be setup by executing
<<aromaSetup1>>=
setupExamples()
@
...
The data used in these examples are largely from our recent paper\footnote{Marcel W. Coolen, Clare Stirzaker, Jenny Z. Song, Aaron L. Statham, Zena Kassir, Carlos S. Moreno, Andrew N. Young, Vijay Varma, Terence P. Speed, Mark Cowley, Paul Lacaze, Warren Kaplan, Mark D. Robinson and Susan J. Clark (2010) Consolidation of the cancer genome into domains of repressive chromatin by long-range epigenetic silencing (LRES) reduces transcriptional plasticity. \textit{Nature Cell Biology}} where we integrated expression and epigenetic data of normal prostate epithelial cells (PrEC) and the LNCaP prostate cancer cell line. The example data includes Affymetrix Human Promoter 1.0R tiling arrays from a histone H3K9 acetylation chromatin immunoprecipitation experiment, matching Affymetrix Human Gene 1.0ST expression arrays, and Illumina sequencing libraries. 

Now, a design matrix, and references to the CDF file and CEL set are created. The CEL set is MAT normalised and MAT smoothed \footnote{Johnson W.E., Li W., Meyer CA., Gottardo R., Carroll J.S., Brown M., and Liu X.S. (2006) Model-based analysis of tiling-arrays for ChIP-chip. \textit{Proc. Natl. Acad. Sci. USA} 103: 12457-12462}.
...
Next, perform a standard Robust Multixchip Analyis (RMA) \footnote{Irizarry R. A., Bolstad B. M., Collin F., Cope L. M., Hobbs, B., Speed, T.P. (2003) Summaries of Affymetrix GeneChip probe level data \textit{Nucleic Acids Research} 31(4):e15} on the example expression data.
...
Next, read in the the positions of human genes (this was adapted from the annotation provided by Affymetrix for their HuGene 1.0ST array platform).
<<readCsv>>=
...
Finally, load the sequencing data. It has been stored compactly in a \texttt{BSgenome::GenomeDataList} object only containing the chromosome, strand and start position of each read (details on how to import data into this format are in the appendix). 
...
\subsection{Plotting Bias Due to CpG Density}
The function \texttt{cpgBoxplots} is used to create boxplots of intensities, binned by the CpG density surrounding 600 bases of the probe. A range of probe GC content can be given, via the \texttt{gcContent} parameter, to narrow down analysis to probes with GC content in a certain range. Each level of GC content is plotted in a separate graph. The \texttt{samples} parameter must be a vector of length 2, that gives the columns of the intensity matrix to use. The number of bins that the CpG density is binned into is controlled with the \texttt{nBins} parameter. Setting the parameter \texttt{calcDiff} to \textbf{TRUE} will create a single plot of the difference of the first sample in the sample vector to the second sample.
\\ \\
\textbf{Examples:}
<<cpgBP1, fig=TRUE>>=
cpgBoxplots(csTU, samples = c(1,2), gcContent = 12, nBins = 10)
@

<<cpgBP2, fig=TRUE>>=
cpgBoxplots(csTU, samples = c(2,1), gcContent = 12, nBins = 10, calcDiff = TRUE)
@

\subsection{Binned Plots}
Binned plots are a way of exploring the relationship between tiling array data

It is possible to bin the tiling array data by the level of expression from another array experiment. The first type of plot to visualise this relationship with is the lineplot. Each line is a different expression bin, and the y-axis is the level of the mark measured by the tiling array. The other type of plot is the heatmap, where the y-axis is the level of expression, and the colour of the square is the level of mark.
\\ \\
The expression data is first put through a standard robust multichip analysis. Then, the following steps are needed to match the tiling array probes to relative positions around the TSSs of the expression data.
\\ \\

\textbf{Example:}


<<binPlots1, eval=FALSE>>=
lookupT <- binPlots(csTS, coordinatesTable=genes, orderingList=list("PrEC Expression"=emE[,"PrEC"], "LNCaP Expression"=emE[,"LNCaP"], "LNCaP-PrEC Expression"=emE[,"LNCaP-PrEC"]), plotType="line", nbins=10)
#aroma.affymetrix maintenance
getNames <- function(...) UseMethod("getNames")
@

<<binPlots1a, fig=TRUE, echo=FALSE>>=
lookupT <- binPlots(extract(csTS,1), coordinatesTable=genes, orderingList=list("PrEC Expression"=emE[,"PrEC"]), plotType="line", nbins=10)
#aroma.affymetrix maintenance
getNames <- function(...) UseMethod("getNames")
@

<<binPlots1b, fig=TRUE, echo=FALSE>>=
binPlots(extract(csTS,2), probeMap=lookupT, orderingList=list("LNCaP Expression"=emE[,"LNCaP"]), plotType="line", nbins=10)
@

<<binPlots1c, fig=TRUE, echo=FALSE>>=
binPlots(extract(csTS,3), probeMap=lookupT, orderingList=list("LNCaP-PrEC Expression"=emE[,"LNCaP-PrEC"]), plotType="line", nbins=10)
@

The first time \texttt{binPlots} is called, it will create a mapping between the gene positions supplied and the probes on the tiling array, which is returned. Keeping this mapping and passing it to subsequent \texttt{binPlots} calls as the \texttt{probeMap} parameter will massively speed up plotting.
...
<<binPlots2, fig=TRUE>>=
binPlots(extract(csTS,1), probeMap=lookupT, orderingList=list("PrEC Expression"=emE[,"PrEC"]), plotType="heatmap", nbins=50)
@

<<binPlots3, fig=TRUE>>=
binPlots(extract(csTS,1), probeMap=lookupT, orderingList=list("PrEC Expression"=emE[,"PrEC"]), plotType="terrain", nbins=50)
@

<<binPlots4, fig=TRUE>>=
binPlots(rs, coordinatesTable=genes, design=designSeq[,"LNCaP IP", drop=FALSE], orderingList=list("LNCaP Expression"=emE[,"LNCaP"]), plotType="heatmap", nbins=50, seqLen=300, libSize="lane")
@
...
significanceGraphs(extract(csTS, 3), probeMap=lookupT, geneList=geneList, titles="H3K9ac Change Across Promoters")
@

And again for sequencing data:

<<sigGraphs2, fig=TRUE>>=
significanceGraphs(rs, coordinatesTable=genes, design=designSeq[,"LNCaP-PrEC IP", drop=FALSE], geneList=geneList, seqLen=300, titles="IP Change Across Promoters")
@
