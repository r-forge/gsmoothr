...
For questions that are not discussed in the function documentation, \texttt{Repitools} has a searchable mailing list, which can be accessed from (authentication required): \\
